...
public class InteractButton : MonoBehaviour, IPointerDownHandler, IPointerUpHandler
{
    ...
    private string defaultText = "Взять";
    private string putText = "Поставить";
    ...
    private void Start()
    {
        ...
        if (radialImage != null)
        {
            ...
            if (debug)
            {
                Debug.Log($"[InteractButton] Радиальный прогресс инициализирован: fillAmount = {radialImage.fillAmount}");
            }
        }
    }
    ...
    private void Update()
    {
        ...
        if (gameObject.activeInHierarchy)
        {
            ...
            if (isPressed)
            {
                ...
                else if (debug && Time.frameCount % 30 == 0)
                {
                    Debug.LogWarning("[InteractButton] Update: isPressed=true, но currentInteractableObject=null");
                }
            }
        }
    }
    ...
    private void FindComponents()
    {
        ...
        if (debug)
        {
            if (radialImage == null)
            {
                Debug.LogWarning("[InteractButton] Radial Image не найден! Убедитесь, что в иерархии есть Image с Type=Filled и FillMethod=Radial360");
            }
            else
            {
                Debug.Log($"[InteractButton] Radial Image найден: {radialImage.name}, raycastTarget: {radialImage.raycastTarget}");
            }
            if (textTMP == null)
            {
                Debug.LogWarning("[InteractButton] Text_tmp (TextMeshProUGUI) не найден! Убедитесь, что в иерархии есть TextMeshProUGUI");
            }
        }
    }
    ...
    private void FindPlayer()
    {
        if (playerTransform == null)
        {
            ...
            if (player != null)
            {
                ...
                if (debug)
                {
                    Debug.Log($"[InteractButton] Игрок найден: {playerTransform.name}");
                }
            }
            else if (debug)
            {
                Debug.LogWarning("[InteractButton] Игрок не найден! Проверьте тег 'Player'");
            }
        }
    }
    ...
    private void FindPlayerCarryController()
    {
        if (playerCarryController == null)
        {
            ...
            if (debug && playerCarryController != null)
            {
                Debug.Log($"[InteractButton] PlayerCarryController найден: {playerCarryController.name}");
            }
        }
    }
    ...
    private void FindNearestInteractableObject()
    {
        if (playerTransform == null)
        {
            ...
            if (playerTransform == null)
            {
                ...
                if (debug)
                {
                    Debug.Log("[InteractButton] FindNearestInteractableObject: playerTransform == null");
                }
                return;
            }
        }
        ...
        if (activePanel != null && activePanel.HasUI())
        {
            ...
            if (debug)
            {
                Debug.Log($"[InteractButton] Найден активный PlacementPanel: {activePanel.name} (HasUI: true)");
            }
        }
        else
        {
            ...
            foreach (InteractableObject obj in allObjects)
            {
                ...
                if (isCarried)
                {
                    ...
                    if (debug)
                    {
                        Debug.Log($"[InteractButton] Найден переносимый объект: {obj.name} (isCarried: true)");
                    }
        ...
        if (currentInteractableObject != closestObject)
        {
            ...
            if (debug)
            {
                if (currentInteractableObject != null)
                {
                    Debug.Log($"[InteractButton] Ближайший объект найден: {currentInteractableObject.name}, расстояние: {closestDistance:F2}, interactionTime: {interactionTime}");
                }
                else
                {
                    Debug.Log("[InteractButton] Ближайший объект не найден");
                }
            }
        }
    }
    ...
    private void UpdateButtonVisibility()
    {
        ...
        // Логируем только при изменении состояния видимости
        if (debug && shouldShow != lastShouldShowState)
        {
            BrainrotObject brainrotObj = currentInteractableObject as BrainrotObject;
            bool isCarried = brainrotObj != null && brainrotObj.IsCarried();
            Debug.Log($"[InteractButton] Видимость кнопки: {shouldShow} (currentObject: {currentInteractableObject?.name ?? "null"}, isCarried: {isCarried})");
            lastShouldShowState = shouldShow;
        }
    }
    ...
    private void UpdateButtonText()
    {
        ...
        if (textTMP.text != newText)
        {
            ...
            if (debug)
            {
                Debug.Log($"[InteractButton] Текст кнопки обновлен: {newText} (hasBrainrotInHands: {hasBrainrotInHands})");
            }
        }
    }
    ...
    private void UpdateProgress()
    {
        if (radialImage == null)
        {
            ...
            {
                Debug.LogWarning("[InteractButton] UpdateProgress: radialImage == null");
            }
            return;
        }
        ...
        if (currentInteractableObject == null)
        {
            if (debug && Time.frameCount % 30 == 0)
            {
                Debug.LogWarning("[InteractButton] UpdateProgress: currentInteractableObject == null, сбрасываем isPressed");
            }
            isPressed = false;
            ResetProgress();
            return;
        ...
        // Сохраняем состояние до обновления (для проверки завершения)
        float previousHoldTime = currentInteractableObject.GetCurrentHoldTime();
        
        // ДИАГНОСТИКА: Проверяем состояние перед обновлением
        if (debug && Time.frameCount % 30 == 0)
        {
            // Проверяем isHoldingKey через рефлексию (если возможно) или добавляем публичный метод
            Debug.Log($"[InteractButton] UpdateProgress: ПЕРЕД UpdateMobileInteraction: previousHoldTime={previousHoldTime:F3}, deltaTime={deltaTime:F3}");
        }
        ...
        // ДИАГНОСТИКА: Проверяем изменение после обновления
        if (debug && Time.frameCount % 30 == 0)
        {
            Debug.Log($"[InteractButton] UpdateProgress: ПОСЛЕ UpdateMobileInteraction: currentHoldTime={currentHoldTimeFromObject:F3}, изменился: {(Mathf.Abs(currentHoldTimeFromObject - previousHoldTime) > 0.001f ? "ДА" : "НЕТ")}");
        }
        ...
        if (debug && Time.frameCount % 10 == 0)
        {
            Debug.Log($"[InteractButton] UpdateProgress: currentHoldTime={currentHoldTimeFromObject:F3}, fillAmount={fillAmount:F3}, interactionTime={interactionTime:F3}");
        }
        ...
        if (currentHoldTimeFromObject >= interactionTime)
        {
            ...
            if (debug)
            {
                Debug.Log($"[InteractButton] UpdateProgress: Взаимодействие завершено (currentHoldTime >= interactionTime), fillAmount установлен в 1.0, isPressed сброшен");
            }
        }
    }
    ...
    public void OnPointerDown(PointerEventData eventData)
    {
        ...
        if (currentInteractableObject == null)
        {
            if (debug)
            {
                Debug.LogWarning("[InteractButton] OnPointerDown: currentInteractableObject == null");
            }
            return;
        }
        ...
        if (isPressed)
        {
            if (debug)
            {
                Debug.LogWarning("[InteractButton] OnPointerDown: isPressed уже true, игнорируем повторный вызов");
            }
            return;
        }
        ...
        if (debug)
        {
            Debug.Log($"[InteractButton] OnPointerDown: Начато взаимодействие с {currentInteractableObject.name}, interactionTime: {interactionTime}, isPressed: {isPressed}");
        }
    }
    ...
    public void OnPointerUp(PointerEventData eventData)
    {
        ...
        if (!isPressed)
        {
            if (debug)
            {
                Debug.LogWarning("[InteractButton] OnPointerUp: isPressed уже false, игнорируем вызов");
            }
            return;
        }
        ...
        if (debug)
        {
            Debug.Log($"[InteractButton] OnPointerUp: Взаимодействие прервано, isPressed: {isPressed}");
        }
    }
