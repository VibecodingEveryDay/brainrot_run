...
public class ShopSpeedManager : MonoBehaviour
{
    ...
    private void UpdateSpeedBar(SpeedBar bar, int currentLevel, float currentSpeed)
    {
        ...
        if (bar.priceText != null)
        {
            ...
            else
            {
                ...
                if (debug)
                {
                    Debug.Log($"[ShopSpeedManager] Bar '{bar.barName}' - Уровень: {currentLevel}, PriceText обновлен: {bar.priceText.text} (raw price: {price})");
                }
            }
        }
        else if (debug)
    ...
    private void OnBuySpeedButtonClicked(int speedAmount)
    {
        ...
        if (price >= 1000)
        {
            // Если цена большая, конвертируем в формат с множителем
            double priceDouble = (double)price;
            (priceValue, priceScaler) = ConvertDoubleToPrice(priceDouble);
        }
        
        bool purchaseSuccess = gameStorage.SubtractBalanceWithScaler(priceValue, priceScaler);
